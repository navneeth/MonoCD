...
class ZODDataset(Dataset):
	def __init__(self, cfg, root, is_train=True, transforms=None, augment=True):
		super(ZODDataset, self).__init__()
		self.root = root
		print('---------------------zod root:', root)
		...
		self.image_right_dir = os.path.join(root)
		self.label_dir = os.path.join(root)
		...
		self.image_files = [os.path.join(self.root, f) for f in os.listdir(self.root) if f.endswith('.png')]
	...
	def get_image(self, idx):
		...
		resized_img = img.resize((1280, 384))
		return resized_img
	...
	def get_calibration(self, idx, use_right_cam=False):
		...
		calib_filename = os.path.join(self.calib_dir, 'default_calib.txt')
	...
	def __getitem__(self, idx):
		...
		if idx >= self.num_samples:
			...
			if self.use_ground_plane or self.pred_ground_plane:
				ground_plane = None 
    			#if self.split == 'test' else self.get_ground_planes(idx)
